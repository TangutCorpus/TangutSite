<template>
  <div class="card-container relative max-h-[50vh] overflow-hidden flexbox-center">
    <button class="absolute left-2 center-y button-helper"
            @click="prevImage">◀
    </button>
    <img src="/home/witbore/Downloads/6_-_0313.webp" class="w-auto max-h-full mx-auto cursor-pointer rounded-lg"
         @click="$emit('open-slider')">
    <button class="absolute right-2 center-y button-helper"
            @click="nextImage">▶
    </button>
  </div>
</template>

<script setup>
import {ref} from 'vue'

const props = defineProps({images: Array})
const currentImage = ref(0)
const prevImage = () => currentImage.value = (currentImage.value - 1 + props.images.length) % props.images.length
const nextImage = () => currentImage.value = (currentImage.value + 1) % props.images.length
</script>