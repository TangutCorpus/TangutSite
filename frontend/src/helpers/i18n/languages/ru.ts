const ru = {
    common: {
        loading: "Загрузка...",
        save: "Сохранить",
        saving: "Сохранение...",
        cancel: "Отмена",
        hide: "Скрыть",
        delete: "Удалить"
    },
    languages: {
        ru: "Русский",
        en: "Английский",
        zh_tw: "Китайский (традиционный)",
        zh_cn: "Китайский (упрощёный)",
    },
    DictionaryArticlePage: {
        title: "Словарная статья",
        edit: "Редактировать",
        loading: "Загрузка статьи..."
    },
    CharacterCard: {
        key: "Ключ",
        strokes: "черт",
        components: "Компоненты",
        seaOfWriting: "Анализ «Моря письмен»",
        seaOfWritingHint: "Анализ структуры иероглифа согласно древнему словарю «Море письмен» (Вэньхай).",
        roles: {
            semantic: "Семантик",
            phonetic: "Фонетик",
            chinesePhonetic: "Кит. фонетик",
            other: "Другое"
        }
    },
    RawDataCard: {
        tabs: {
            main: "Основное",
            correspondences: "Соответствия"
        },
        strokeCountTotal: "Общее кол-во черт",
        initials: "Инициали",
        tone: "Тон",
        rhymes: "Рифмы",
        rhymeLabel: "Рифма {number}",
        fanqie: "Фаньце",
        chineseCharacters: "Кит. эквиваленты",
        tibetanSyllables: "Тибетские слоги",
        sanskritSyllables: "Санскритские слоги"
    },
    ReconstructionsCard: {
        title: "Реконструкции",
        deprecated: "устар.",
        hideDeprecated: "Скрыть устаревшие",
        showDeprecated: "Показать устаревшие ({count})"
    },
    DictionaryReferencesCard: {
        title: "Ссылки на словари",
        remove: "Удалить ссылку",
        add: "Добавить ссылку",
        selectDict: "Выберите словарь",
        pageOrNumber: "Стр. / Номер",
        save: "Добавить"
    },
    CharacterImagesCard: {
        title: "Формы иероглифа в текстах"
    },
    CompoundWordsCard: {
        title: "Составные слова"
    },
    RelatedWordsCard: {
        title: "Родственные слова (Гьялронгские)"
    },
    CorpusExamplesCard: {
        title: "Примеры из корпуса"
    },
    DictionaryEdit: {
        createTitle: "Создание словарной статьи",
        editTitle: "Редактирование статьи",
        sections: {
            basic: "Основная информация",
            components: "Разбиение на элементы",
            phonology: "Фонетика и структура",
            compoundWords: "Составные слова"
        },
        character: "Иероглиф",
        unicodeCode: "Код Юникода",
        unicodeKey: "Ключ Юникода",
        strokeCountUnicode: "Черт (Unicode)",
        strokeCountTotal: "Черт (Общее)",
        tone: "Тон",
        seaOfWriting: "Море письмен (4 знака)",
        linkedArticleId: "ID связанной статьи",
        addComponent: "Добавить элемент",
        compoundText: "Слово",
        compoundMeaning: "Значение",
        enterMeaning: "Введите перевод слова",
        addCompoundWord: "Добавить составное слово",
        note: "Примечание"
    },
    About: {
        title: "Тангутское Наследие",
        subtitle: "2026",
        historyTitle: "Забытая письменность",
        historyP1: "Тангутское письмо (си-ся) — одна из самых сложных и загадочных систем письма в истории человечества. Созданная в XI веке для нужд Великого государства Си Ся, она насчитывает более 6000 уникальных знаков, каждый из которых по сложности может превосходить китайский иероглиф.",
        historyP2: "После падения государства под ударами войск Чингисхана в 1227 году, язык был практически забыт на семь столетий, пока в начале XX века не были обнаружены бесценные рукописи в мертвом городе Хара-Хото.",
        missionTitle: "Миссия нашего проекта",
        missionText: "Мы создаем цифровую среду для дешифровки и изучения тангутских текстов. Наша цель — сделать древние манускрипты доступными для исследователей и лингвистов нового поколения, сочетая филологическую точность с современными технологиями обработки данных.",
        featuresTitle: "Особенности системы",
        features: [
            "Полное отсутствие заимствованных знаков",
            "Сверхсложная идеографическая структура",
            "Уникальные буддийские канонические тексты",
            "Логическая система построения радикалов"
        ],
        stats: {
            characters: "знаков",
            year: "год",
            texts: "текстов"
        }
    },
    AuthComponent: {
        email: 'Email',
        nickname: 'Никнейм',
        password: 'Пароль',
        next: 'Дальше',
        realName: 'ФИО',
        biography: 'О себе',
        agreeWithTerms: 'Я согласен с условиями',
        completeRegistration: 'Завершить регистрацию',
        signIn: 'Войти',
        authDescription: 'Зарегистрируйтесь или войдите, чтобы получить полный доступ',
        registrationHeader: 'Регистрация',
        signInHeader: 'Вход'
    }, HeaderNavComponent: {
        dictionary: 'Словарь',
        aboutPage: 'О нас',
        library: 'Библиотека',
        signIn: 'Вход',
        addText: 'Добавить текст',
        profile: 'Профиль'
    }, SearchBar: {
        placeholder: "Введите запрос…",
        searchArea: "Область поиска",
        textMode: "Тексты",
        dictionaryMode: "Словарь",
        chooseRadicals: "Выбрать компоненты"
    }, TextFragmentEdit: {
        page: "Страница",
        textId: "ID текста"
    }, ToggleButtons: {
        registration: "Регистрация",
        signIn: "Вход"
    }, HomePage: {
        title: "Корпус тангутского языка"
    }, MainLibraryPage: {
        contactAdministrator: "Обратитесь к администратору",
        textsNotFound: "Тексты не найдены"
    }, NotFoundPage: {
        notFound: "Страница не найдена",
        goBackToHomePage: "Вернуться на главную"
    }, SearchResultCard: {
        readFarther: "Читать далее",
    }, TextFragmentEditPopup: {
        save: "Сохранить",
        close: "Закрыть"
    }, TextFragmentPreview: {
        edit: "Редактировать",
        error: "Ошибка",
        notAccessible: "Страница недоступна…",
        page: "Страница",
    }, TextMetadataEdit: {
        textMetadata: "Метаданные текста",
        deleteProperty: "Удалить свойство",
        choosePropertyToDelete: "Выберите редактируемое свойство",
        author: "Автор",
        date: "Дата создания",
        description: "Описание",
        material: "Материал",
        storage: "Хранение",
        catalog: "Каталогизация",
        repository: "Место хранения",
        form: "Форма",
        height: "Высота",
        width: "Ширина",
        frameType: "Тип рамки",
        frameHeight: "Высота рамки",
        pages: "Количество страниц",
        linesPerPage: "Строк на странице",
        charactersPerLine: "Иероглифов в строке"
    }, TextEditPage: {
        addFragment: "Добавить фрагмент",
        textName: "Название текста",
        toSave: "Сохранить",
        saved: "Сохранено"
    }, TranslationListEdit: {
        editTranslations: "Редактировать переводы",
        addTranslation: "Добавить перевод",
        originalText: "Оригинальный текст",
        enterTranslation: "Введите перевод"
    }, TextFragmentEditPage: {
        save: "Сохранить"
    }, TranslationList: {
        original: "Оригинал",
        translations: "Переводы",
        error: "Ошибка",
        noTranslations: "Нет доступных переводов…",
    }, TextFragmentPage: {
        text: "Текст",
        page: "Страница",
        edit: "Редактировать"
    }, GlossedText: {
        error: "Ошибка",
        noConnectedPages: "Нет связанных с этим текстом страниц",
        page: "Страница"
    }, TextCommentCard: {
        comments: "Комментарии"
    }, TextPage: {
        information: "Информация",
        edit: "Редактировать"
    }, ImageUpload: {
        images: "Изображения",
        pickImage: "Выбрать изображения"
    }, UserAvatarUpload: {
        changeAvatar: "Изменить аватар"
    }, UserBiographyEdit: {
        biography: "О пользователе"
    }, UserContactEdit: {
        contactInfo: "Контактные данные",
        email: "Email",
        realName: "ФИО"
    }, UserEditPage: {
        save: "Сохранить"
    }, UserBiography: {
        biography: "О пользователе"
    }, UserContact: {
        contactInfo: "Контактные данные",
        email: "Email",
        realName: "ФИО",
        role: "Права"
    }, DictionaryIndexPage: {
        title: "Словарь",
        totalCount: "Всего статей"
    }
}

export default ru